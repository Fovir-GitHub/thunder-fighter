...
public class CollisionDetector { // for every frame detection, the passed values should be a List
  public static void detectBulletAircraftCollisions(List<Bullet> bullets, List<Aircraft> aircrafts) {
    for(Bullet bullet : bullets) {
      ...
      for(Aircraft aircraft : aircrafts) {
        ...
        if((bullet.isFromPlayer() && !aircraft.isPlayer()) || (!bullet.isFromPlayer() && aircraft.isPlayer())) {
    ...
    for(int i = 0; i < aircrafts.size(); i++) {
      ...
      for(int j = i + 1; j < aircrafts.size(); j++) {
        ...
        if(b1.intersects(b2)) {
          a1.takeDamage(1);
          a2.takeDamage(1);
        } // enemy and player all deduct 1 hp, although it's a stupid action lol, because player only have 3 hp.
      }
    }
  } // aircraft <-> aircraft
}
